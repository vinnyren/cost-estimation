...
async def test_calc_with_factors_differs_from_default(client_factory):
...
async def test_calc_warning_when_factors_null(client_factory):
    async with await client_factory() as client:
...
async def test_calc_no_warning_when_factors_present(client_factory):
    async with await client_factory() as client:
        pid = await _seed(
            client,
            factors_dev={
...
async def test_calc_reverse_picks_up_factors(client_factory):
