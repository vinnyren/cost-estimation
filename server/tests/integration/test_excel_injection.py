...
async def client(monkeypatch, tmp_path):
    ...
    for m in ["app.config", "app.db.session", "app.deps", "app.db.models",
              ...
              "app.services.functions", "app.services.reports",
              "app.exporters.excel", "app.exporters.fallback",
              "app.api.projects", "app.api.params", "app.api.calc",
              "app.api.functions", "app.api.reports", "app.api.health", "app.main"]:
...
async def test_evil_fp_name_does_not_become_formula(client, evil):
...
async def test_normal_fp_name_unchanged(client):
    """非公式串不受影响。"""
...
async def test_evil_project_name_in_cover_sheet(client):
