...
async def test_effective_returns_flat_keys_with_overrides_empty(client):
...
async def test_effective_404_for_missing_project(client):
    r = await client.get(
...
async def test_override_applies_and_appears_in_effective(client):
    pid = await _make_project(client)
...
async def test_override_supports_productivity_flat_path(client):
    pid = await _make_project(client)
    r = await client.patch(
        f"/api/projects/{pid}/params/override",
        headers={**H, "Content-Type": "application/json"},
...
async def test_override_clear_via_null(client):
    pid = await _make_project(client)
    await client.patch(
        f"/api/projects/{pid}/params/override",
        headers={**H, "Content-Type": "application/json"},
...
async def test_global_reset_clears_modified_and_reseeds(client):
...
async def test_calc_forward_uses_overrides(client):
...
async def test_override_404_for_missing_project(client):
    r = await client.patch(
