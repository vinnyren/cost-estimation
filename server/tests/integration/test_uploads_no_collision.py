...
async def test_same_filename_uploaded_twice_does_not_overwrite(client_factory, tmp_data_dir):
    async with await client_factory() as c:
        ...
        # parsed paths are stored relative to settings.parsed_dir = tmp_data_dir/parsed
        abs1 = tmp_data_dir / "parsed" / p1
        abs2 = tmp_data_dir / "parsed" / p2
        assert abs1.exists() and abs2.exists(), "both parsed-text files must persist"


async def test_same_basename_diff_extension_does_not_collide(client_factory, tmp_data_dir):
    async with await client_factory() as c:
        pid = await _make_project(c)
        r1 = await _upload(c, pid, "sample.pdf", "needs.pdf")
...
async def test_uploaded_files_listed_separately(client_factory, tmp_data_dir):
    async with await client_factory() as c:
        pid = await _make_project(c)
...
async def test_invalid_upload_does_not_leak_blob_on_disk(client_factory, tmp_data_dir):
    async with await client_factory() as c:
        pid = await _make_project(c)
        r = await c.post(
            f"/api/projects/{pid}/uploads",
