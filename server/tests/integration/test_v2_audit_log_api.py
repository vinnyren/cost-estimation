...
async def test_project_patch_writes_audit_log(client_factory):
    async with await client_factory() as client:
        pid = await _make_project(client)
        await client.patch(
...
async def test_get_does_not_write_audit_log(client_factory):
    async with await client_factory() as client:
        pid = await _make_project(client)
...
async def test_audit_log_cursor_pagination(client_factory):
    async with await client_factory() as client:
        pid = await _make_project(client)
        for i in range(5):
            await client.patch(
