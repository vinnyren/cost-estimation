...
> 版本：v2.6.0 · 适用 plugin v2.6.0 · 数据基准 **CSBMK®-202510**
> 编写日期：2026-05-11 · 最后更新：2026-05-18 · 适用方法学：NESMA 估算法（兼容 IFPUG 详细计数）
...
> v2.6 新增：反算重做、6-sheet 报告、500MB 上传等 11 项（详见[第 13 章](#第-13-章-v26-新特性使用说明)）
...
- [第 13 章 v2.6 新特性使用说明](#第-13-章-v26-新特性使用说明)
- [附录 A 公式速查](#附录-a-公式速查)
- [附录 B 词汇表](#附录-b-词汇表)
...
- **目标总造价（万元）**：v2.6 起按万元填写，如 `50` 表示 50 万元（计算层内部仍按元运算）
- **项目类型**：选择是否含运维；v2.6 反算不再需要手填 α 开发占比，开发/运维成本拆分由系统自动推导
...
> 版本说明：v2.6（2026-05-18）在 v2.5 基础上重做了反算模型、Excel 报告格式与大文件上传。本章用面向最终用户的语气，讲清楚这些变化在实际操作里怎么用。v2.6 不改动数据库结构，老项目无需迁移即可继续使用。

### 13.1 反算怎么用了（单一规模 + 自动计算）

v2.6 之前，反向项目把目标造价拆成开发、运维两份分别反推，会得到两个不同的功能点规模，而 FP 表只能保存一个，导致「反算出来的数字」和「正向再算一遍的总额」对不上。v2.6 改为开发与运维**共用同一个功能点规模**，三档（乐观 / 中位 / 保守）各自只有一个规模数字，正向复算的总成本与目标造价一致。

新的操作流程：

1. **创建反向项目**：向导里选「反向」，目标总造价按**万元**填写（如填 `50` 表示 50 万元，不再填 `500000`）。「其他费用」同样按万元填。
2. **进结果页自动反算**：只要项目已经有目标造价，进入结果页时系统**自动完成反算**，不需要再手动点「反算」按钮。
3. **看三档规模**：结果页给出乐观 / 中位 / 保守三档可承载的功能点规模，推荐采纳中位档。每档下方还会显示开发成本与运维成本的拆分（这是按生产率/费率自动推导出来的，不再需要你设置 α 开发占比）。
4. **看「反算 UFP 模块细化分摊」表**：系统读取项目现有 FP 表，按各一级模块当前的 UFP 占比，把反算出的目标 UFP 总量分摊到每个模块。表里每行给出该模块的「现有 UFP / 分摊后 UFP / 需细化增加的差额」，差额就是你后续要在该模块里补充设计的功能点工作量。

> 💡 如果项目还没有 FP 表，先上传需求文档并用 AI 提取出初稿，模块细化分摊才有占比基础。

### 13.2 「写回 FP 表」的规模量级提示

在反算分摊面板（AllocatorPanel）里，模块清单现在直接来自项目真实功能点的一级模块，不再要手填 JSON。当你点「写回 FP 表」时，确认框会显示规模**写回前与写回后的量级对比**：

- 若变化幅度不大，正常确认即可。
- 若规模被剧烈放大或缩小，确认框会弹出**强警告**，提示这次写回可能会冲掉你已有的 FP 设计稿。请先确认是不是目标造价或参数填错了，再决定是否继续。

### 13.3 反向项目的 Excel 报告（6 个 Sheet）

v2.6 之前，导出 Excel 永远走正向计算，反向项目导出的报告并不反映目标造价。现在系统会按项目模式分流：**反向项目导出的报告，总费用就等于你设定的目标造价**。

报告重做为 6 个 Sheet 的行业评估表格式：

| Sheet | 名称 | 内容 |
|---|---|---|
| 1 | 封面 | 项目与评估机构信息、报告声明、生成日期 |
| 2 | 评估结果汇总 | 规模、工作量、成本、造价主表（三档汇总） |
| 3 | 模块功能点及费用分项统计表 | 各一级模块的功能点数量与对应费用拆分 |
| 4 | 系统功能点明细表 | 完整 FP 清单（子系统 / 模块 / 类别 / UFP / 规模） |
| 5 | 评估报告书 | 六章文字叙述（项目概述、评估目的、依据、方法、过程、结论） |
| 6 | 调整因子表 | 全部调整因子的取值与说明 |

> 📌 旧版「评估报告书」Sheet 时常是空白的，v2.6 已修复，第 5 个 Sheet 现在会完整呈现六章评估叙述。

### 13.4 大文件上传（最多 500MB）

上传需求文档的大小上限从 50MB 提升到 **500MB**：

- 大文件会分块流式写入磁盘，不会一次性占用大量内存，适合上传大型可研报告、扫描件等。
- 上传超时放宽到 10 分钟，网络较慢时也能完成。
- 如果上传失败，弹窗会显示**真实原因**（例如「文件超过 500MB」），不再只显示一串看不懂的「status code 4xx」。看到提示后按提示处理即可（如压缩文件、拆分文档）。

### 13.5 编辑设定里改正向 / 反向

在项目的 ⋯ 菜单或 FP 编辑页工具栏点「⚙️ 编辑设定」，现在可以修改**评估方式（正向 / 反向）**和**项目类型**并正常保存生效。此前版本里把项目从正向改成反向后不生效的问题已修复。改完保存后，重新进入结果页即按新模式计算。

### 13.6 上传文档后用「AI 任务面板」发起提取

上传需求文档后，页面会出现一条引导提示，告诉你可以让 AI 自动提取功能点。提示条里直接带一个「打开 AI 任务面板」按钮：

1. 上传完文档，点提示条里的「打开 AI 任务面板」（也可直接点页面右上角的「AI 任务面板」）。
2. 在面板里一键发起 AI 提取任务。
3. 面板会实时显示任务进度与阶段日志，完成后回到 FP 编辑表即可看到 AI 生成的功能点初稿，再做微调。

### 13.7 界面用词变化：「US」改为「规模」

FP 编辑表、反算分摊面板、结果页里原来显示的「US」字样，现在统一改为中文「**规模**」，含义不变，便于非技术用户理解。
...
> **文档版本**：v2.6.0 · 最后更新 2026-05-18  
> **本版本变更**：v2.6 重做反算单一规模模型、6-sheet 行业评估报告、500MB 大文件上传等 11 项，详见第 13 章  
> **下一版本计划**：补充 COSMIC 方法、Excel 批量导入示例、移动端适配
